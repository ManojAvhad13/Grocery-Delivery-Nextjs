...
import { Boxes, ClipboardCheck, Cross, LogIn, LogOut, LucideShoppingBasket, Menu, Package, Plus, Search, User, X } from 'lucide-react'
    ...
    const sideBar = menuOpen ? createPortal(
        <AnimatePresence>
            <motion.div
                ...
                className="fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-[9999] bg-gradient-to-b from-green-800/90 via-green-700/80 to-green-900/90
                backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white"
            >
                ...
                <div className="flex items-center gap-4 p-3 pt-3 mt-3 rounded-xl
bg-white/10 hover:bg-white/15 transition-all shadow-inner">
...
border border-green-400/60 shadow-lg">
                        {user.image ? (
                            ...
                            <User className="w-full h-full p-2 text-green-300" />
                        )}
                    </div>
                ...
                p-3 rounded-lg transition-all' onClick={async () => await signOut({ callbackUrl: "/" })}>
    ...
    return (
        <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
            <Link href={"/"} className='text-white font-bold text-2xl sm:text-3xl tracking-wide hover:scale-105 transition-transform'>
                GroceryCart
            </Link>
            ...
            <div className='flex items-center gap-3 md:gap-6 relative'>
                ...
                {user.role == "user" && <>
                    ...
                    <Link href={"/user/cart"} className='relative bg-white rounded-full w-11 h-11 flex items-center justify-center shadow-md hover:scale-105 transition'>
                        <LucideShoppingBasket className='text-green-600 w-6 h-6' />
                ...
                {user.role == "admin" && <>
                    ...
                    <div className='md:hidden bg-white rounded-full w-10 h-10 flex 
                    items-center justify-center shadow-md' onClick={() => setMenuOpen(prev => !prev)}>
                ...
                <div className='relative' ref={profileDropDown}>
                    <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform' onClick={() => setOpen(prev => !prev)}>
                        {user.image ? <Image src={user.image} alt='userProfile' fill className='object-cover rounded-full' /> : <User />}
                    </div>
                    ...
                    <AnimatePresence>
                        ...
                        >
                            ...
                            <button className='flex items-center gap-2 w-full text-left px-3 py-2 hover:bg-red-50 rounded-lg text-gray-700 font-medium' onClick={() => {
                                setOpen(false)
