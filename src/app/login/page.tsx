...
const Login = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative">
            ...
            <p className="text-gray-600 mb-8 flex items-center">Login to Grocery app <Leaf className="w-5 h-5 text-green-600" /></p>
            <motion.form
                ...
                <div className="w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-50 
                py-3 rounded-xl text-gray-700 font-medium transition-all duration-200" onClick={() => signIn("google", { callbackUrl: "/" })}>
                    <Image src={googleImage} width={20} height={20} alt="googleimg" />
                    Continue with Google
                </div>
            </motion.form>

            <p className="cursor-pointer font-semibold text-gray-600 mt-6 text-sm flex items-center gap-1" onClick={() => router.push("/register")}>Want to create an account ? <LogIn className="w-4 h-4" /><span className="text-green-600">Sign up</span></p>
        </div>
    )
}
