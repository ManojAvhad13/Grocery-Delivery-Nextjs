...
const CartPage = () => {
    ...
    // Navigation for Proceed to checkout
    const router = useRouter()

    return (
        <div className='w-[94%] sm:w=[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
            ...
            {cartData.length == 0 ? (
                ...
                >
                    ...
                    <p className='text-gray-600 text-lg mb-6'>Your Cart is empty. Add some groceries to conitinue shopping!</p>
                    <Link href={"/"} className='bg-green-600 text-white px-6 py-3 rounded-full hover:bg-green-700 transition-all
                    inline-block font-medium'>Continue Shopping</Link>
                </motion.div>
            ) : (
                        ...
                        <AnimatePresence>
                            {cartData.map((item, index) => (
                                <motion.div
                                    ...
                                    className='flex flex-col sm:flex-row items-center bg-white rounded-2xl shadow-md p-5 hover:shadow-xl transition-all
                                ...
                                >
                                    ...
                                    <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                                        <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border
                                        border-gray-200' onClick={() => dispatch(decreaseQuantity(item._id))}><Minus size={14} className='text-green-700' /></button>
                                        <span className='font-semibold text-gray-800 w-6 text-center'>{item.quantity}</span>
                                        <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border
                                        border-gray-200'onClick={() => dispatch(increaseQuantity(item._id))}><Plus size={14} className='text-green-700' /></button>
                                    </div>
